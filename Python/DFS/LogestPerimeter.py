...
class CityMap:
    ...
    def trackPerimeterLargest(self) -> int:
        max_perim = 0
        for x in range(self.rows):
            for y in range(self.cols):
                if self.mapa[x][y] == TIERRA: # inicio de una Isla
                    perimeter = self.dfs(x, y) # perímetro de ESTA Isla
                    max_perim = max(max_perim, perimeter) # actualizar el máximo
        return max_perim
    ...
    def dfs(self, x:int, y:int) -> int:
        ...
        perimeter = 0
        perimeter += self.dfs(x + 1, y)
        perimeter += self.dfs(x - 1, y)
        perimeter += self.dfs(x, y + 1)
        perimeter += self.dfs(x, y - 1)
        return perimeter
...
print("\n")
print("trackLongestPerimeter")
mapa.printMap()
print(mapa.trackPerimeterLargest())